  ...
  function($) {
      ...
      void 0 === window.vc && (window.vc = {}), window.vc.ShortcodesBuilder = function(models) {
          ...
          render: function(callback, model) {
              ...
              window.vc.setDataChanged(), this.build(shortcodes, callback, model)
          },
          ...
          buildDefaultCss: function(model) {
              ...
              if (model.settings && !hasContentClass) {
                  var hasContentClass = "",
                      modelParams = model.settings.params,
                      defaultCssValue = null,
                      model = model.settings.element_default_class;
                  if (modelParams && modelParams.length)
                      for (var i = 0; i < modelParams.length; i++)
                          if ("css_editor" === modelParams[i].type) {
                              ...
                              break
                          }(hasContentClass = defaultCssValue ? "." + model + "{" + _.reduce(defaultCssValue, function(memo, value, key) {
                              return value ? memo + key + ": " + value + ";" : memo
                          }, "", this) + "}" : hasContentClass) && vc.frame_window && model && vc.frame_window.vc_iframe.setDefaultShortcodeCss(hasContentClass, model)
              }
          }
  ...
  function($) {
      ...
      window.vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
          ...
          hide: function(e) {
              ...
              // nectar addition
                // Bug in core of WPBakery 7.6 where adding a timeout causes TINYMce to not init sometimes and makes
                // element options modal close without reopening when trying to edit settings of another block.
                // i.e. the block settings of the newly click block will not be shown until clicking again.
                // We're removing the timeout all together when the new option for auto save is not enabled.
